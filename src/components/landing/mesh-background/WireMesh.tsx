...
import { WireMeshHorizontal } from "./wire-mesh-horizontal";
...
function buildDotGeo() {
  const dotsPerLine = Math.floor(VERT_SEGS / DOT_EVERY);
  const totalDots = LINE_COUNT * dotsPerLine;
  const positions = new Float32Array(totalDots * 3);
  let i = 0;
  for (let line = 0; line < LINE_COUNT; line++) {
    const norm = line / (LINE_COUNT - 1);
    for (let seg = 0; seg < VERT_SEGS; seg += DOT_EVERY) {
      const v = seg / VERT_SEGS;
...
// ─── Main mesh component ──────────────────────────────────────────────────────
function WireMesh({ theme }: { theme: ThemeKey }) {
...
function CameraRig() {
  ...
  useFrame(({ clock }) => {
    ...
    // Adjust base Z distance based on aspect ratio (move back on portrait screens)
...
export default function WireMeshBackground({ theme, direction = "vertical", showControlPanel = false }: { theme: ThemeKey, direction?: "vertical" | "horizontal", showControlPanel?: boolean }) {
  return (
    <>
      <Leva hidden={!showControlPanel} />
      <Canvas
      ...
      >
        ...
        {direction === "horizontal" ? (
          <WireMeshHorizontal key={theme + "-h"} theme={theme} />
        ) : (
          <WireMesh key={theme + "-v"} theme={theme} />
        )}
        <MusicElements key={theme + "-music-" + direction} theme={theme} direction={direction} />
      </Canvas>
    </>
  );
}
